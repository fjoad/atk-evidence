# Electricity-Theft Paper Reproducibility Audit — Working Memory

**Last updated:** 2026-07-20

## Environment quirks

- Host is Apple M1 Max/macOS; project Python environment is `replication/.venv`.
...
- Run current deterministic tests with `cd replication/src && ../.venv/bin/python -m unittest -v test_attacks.py test_cer_parser.py`.
- Preserve raw files in place and identify them by checksum; derived artifacts belong under `replication/results/`.
...
- GitHub repository name and public/private visibility.
