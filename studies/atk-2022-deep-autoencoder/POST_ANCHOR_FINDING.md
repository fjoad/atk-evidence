...
Status: primary diagnostic round completed; one small explanatory control pending
...
| Quantity | Paper | Optimistic limit, post-ADASYN | Original-row diagnostic limit |
|---|---:|---:|---:|
...
detect at most 29.57855% and must falsely flag at least 44.28282%. A separate
...
This does not identify the contribution as learned: a no-training reconstruction
using additional input statistics might recover the same distinctions. A small
follow-up control is recorded separately before execution to compare trained,
uniform, and simplex-projection scores within the same energy bands.
...
The next useful distinction is between input geometry and learned contribution,
then source-supported alternatives that would actually change the bound's
assumptions. No alternative is inferred from target proximity.
